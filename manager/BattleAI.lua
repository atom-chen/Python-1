...
--
local BattleAI = class("BattleAI")

BattleAI.HEAD_COLUMN_1 = 1		--玩牌区
BattleAI.HEAD_COLUMN_2 = 2
BattleAI.HEAD_COLUMN_3 = 3
BattleAI.HEAD_COLUMN_4 = 4
BattleAI.HEAD_COLUMN_5 = 5
BattleAI.HEAD_COLUMN_6 = 6
BattleAI.HEAD_COLUMN_MAX = 7

BattleAI.HEAD_COLLECT_1 = 8	--集牌区
BattleAI.HEAD_COLLECT_2 = 9
BattleAI.HEAD_COLLECT_3 = 10
BattleAI.HEAD_COLLECT_MAX = 11

BattleAI.HEAD_CHANGE_1 = 12	--切牌区,已打开
BattleAI.HEAD_CHANGE_MAX = 13	--切牌区，未打开

BattleAI.AI_STATUS_INIT = 1 --初始化
BattleAI.AI_STATUS_PLAYING = 2 --游戏中
BattleAI.AI_STATUS_END = 3 --结束

BattleAI.AI_EASY = "easy" --简单难度
BattleAI.AI_NORMAL = "normal" --中等难度
BattleAI.AI_HARD = "hard" --困难难度

--AI逻辑cd
BattleAI.JUDGE_AI_CD = {
	easy = {2,3},
	normal = {1,2},
	hard = {0,1}
}
...
--收牌逻辑cd加速度
BattleAI.JUDGE_COLLECT_CD_RATE = 1

function BattleAI:ctor(manager)
	self.manager_ = manager
	self.aiAutoCollect_ = false
	self.aiStatus_ = BattleAI.AI_STATUS_INIT
	self.aiLevel_ = BattleAI.AI_NORMAL
...
function BattleAI:setDelayEnd()
	self.delayTime_ = self:getRandomBy_(BattleAI.DELAY_END_TIME[self.aiLevel_][1],BattleAI.DELAY_END_TIME[self.aiLevel_][2])
end

function BattleAI:setLevel(level)
	if not level then
		return
	end
	self.aiLevel_ = level
...
function BattleAI:setIs3Draw(b)
	self.isDraw3_ = b
end

function BattleAI:getCollectRandomBy_(a,b)
	local min_ = a
	local max_ = b
	if self.manager_ and self.manager_.score_ then
		...
		local basic_ = 1 + arg_*BattleAI.JUDGE_COLLECT_CD_RATE
		if basic_ < 0.5 then
			basic_ = 0.5
		end
		-- if self.aiLevel_ == BattleAI.AI_HARD then
		-- 	printf("===[%s]==> BattleAI [%s] = %s",tostring(BattleAI.JUDGE_COLLECT_CD_RATE),tostring(arg_),tostring(basic_))
		-- end
...
function BattleAI:getRandomBy_(a,b)
...
function BattleAI:getScore()
	return self.score_
end

function BattleAI:judgeCollectListExistCard(card)
	for i=BattleAI.HEAD_COLLECT_1,BattleAI.HEAD_COLLECT_MAX do
...
function BattleAI:getAICollectList()
	local list_ = {}
	for i=BattleAI.HEAD_COLLECT_1,BattleAI.HEAD_COLLECT_MAX do
		local cList_ = DealerController.getListByHead(self.headsList_[i])
		if #cList_ > 0 then
...
function BattleAI:getAIStatus()
	return self.aiStatus_
end

function BattleAI:ending()
	self.aiStatus_ = BattleAI.AI_STATUS_END
end

function BattleAI:aiEnd(isWin)
	self:ending()
...
function BattleAI:begin()
	self.aiStatus_ = BattleAI.AI_STATUS_PLAYING
	local delay_ = 1 --发牌动画延迟
	self.aiCd_ = delay_ + self:getCollectRandomBy_(BattleAI.JUDGE_COLLECT_CD[self.aiLevel_][1],BattleAI.JUDGE_COLLECT_CD[self.aiLevel_][2])
	-- printf("self.aiCd === %s", tostring(self.aiCd_))
	-- if self.aiAutoCollect_ then
	-- 	self:autoCollect_()
	-- else
	-- 	local b = self:aiCollect_()
	-- 	if b then
	-- 		self.aiCd_ = self:getRandomBy_(BattleAI.JUDGE_COLLECT_CD[self.aiLevel_][1],BattleAI.JUDGE_COLLECT_CD[self.aiLevel_][2])
	-- 	end
	-- end
end

function BattleAI:setDelegate(delegate)
	self.delegate_ = delegate
end

function BattleAI:update(t)
	if self.aiStatus_ ~= BattleAI.AI_STATUS_PLAYING then
		return
	end
...
function BattleAI:initCardByList(list)
	self.headsList_ = {}
	if not list then
		return
	end
	for i=BattleAI.HEAD_COLUMN_1,BattleAI.HEAD_CHANGE_MAX do
		local head_ = list[i]
		local before_ = nil
		while head_ do
...
function BattleAI:copyCard_(card)
...
--初始化牌
function BattleAI:initCardBySeed( _seed )
	self.headsList_ = {}
	if not _seed then
		return
	end
	...
	for i=1,#list do
		local cardVO = list[i]
		linkCard_(before, cardVO, BattleAI.HEAD_CHANGE_MAX)
		cardVO:setProperty("board", CardVO.BOARD_BACK)
		before = cardVO
	end
end
...
--执行AI逻辑(一步)
function BattleAI:doAiLogic()
	self:judgeNeedEmptyCol_()
	--判断是否有牌可以收
	local b = self:aiCollect_()
	if b then
		self.emptyChangeNum_ = 0 --空切牌数(用于判断是否无操作可做)
		self.aiCd_ = self:getCollectRandomBy_(BattleAI.JUDGE_COLLECT_CD[self.aiLevel_][1],BattleAI.JUDGE_COLLECT_CD[self.aiLevel_][2])
		return
	end

	--重置AI cd
	self.aiCd_ = self:getRandomBy_(BattleAI.JUDGE_AI_CD[self.aiLevel_][1],BattleAI.JUDGE_AI_CD[self.aiLevel_][2])
...
--空切的轮数
function BattleAI:getEmptyRound_()
	local changeLen_ = DealerController.getQueueLenByHead(self.headsList_[BattleAI.HEAD_CHANGE_1])
	local changeMaxLen_ = DealerController.getQueueLenByHead(self.headsList_[BattleAI.HEAD_CHANGE_MAX])
	local result_ = 0
...
function BattleAI:judgeWin_()
	local win = true
	--牌桌和换牌区都没有卡牌的时候胜利
	for i=BattleAI.HEAD_COLUMN_1,BattleAI.HEAD_COLUMN_MAX do
	...
	if win then
		if self.headsList_[BattleAI.HEAD_CHANGE_1] or self.headsList_[BattleAI.HEAD_CHANGE_MAX] then
			win = false
		end
	end
	return win
...
function BattleAI:judgePut_(curCard,list)
	if not curCard then
		return false
	end
	for i=BattleAI.HEAD_COLUMN_1,BattleAI.HEAD_COLUMN_MAX do
...
--切牌区是否有牌可以落到curCard
function BattleAI:judgeCanChangePutOn_(curCard,list)
	if not curCard then
		return false
	end
	--如果点数小于3则不考虑
...
BattleAI.sameCard_yes_can_collect = 1 --有，但是可以通过收牌来消除
BattleAI.sameCard_no = 2 --没有
BattleAI.sameCard_yes = 3 --有，且暂时无法消除

function BattleAI:judgeSameCard_(curCard,list)
	if not curCard then
		return BattleAI.sameCard_yes
	end
	...
	if sameCard_ then
		if sameCard_:getNextCard() then --不是列尾
			...
			if isCanCollect_ then
				return BattleAI.sameCard_yes_can_collect
			else
				return BattleAI.sameCard_yes
			end
		else --是列尾
	...
	return BattleAI.sameCard_no
end

--玩牌区是否有不是头牌的K
function BattleAI:judgeNotHeadKCard_(list)
	for i=BattleAI.HEAD_COLUMN_1,BattleAI.HEAD_COLUMN_MAX do
		local card = list[i]
		--该队列第一张正面的牌
		card = DealerController.getFirstFaceCardFromHead(card)
...
function BattleAI:getChangeCardBy_(list,rank,color)
	local list_ = {}

	--切牌区(打开)
	local changeHead_ = self.headsList_[BattleAI.HEAD_CHANGE_1]
	...
	--切牌区(未打开)
	local changeMaxHead_ = self.headsList_[BattleAI.HEAD_CHANGE_MAX]
...
function BattleAI:judgeCollect_(curCard,list)
...
function BattleAI:autoCollect_()
...
function BattleAI:collect_()
	...
	--切牌区(已打开)
	local card = DealerController.getQueueEndCardVO(self.headsList_[BattleAI.HEAD_CHANGE_1])
	if card then
		local aiMoveVO_ = self:judgeCollect_(card,self.headsList_)
		if aiMoveVO_ then
			return aiMoveVO_
...
--头牌是K
function BattleAI:isHeadK_(card)
...
function BattleAI:judgeMove_(curCard,list,ignoreCol)
	if not curCard then
		return
	end
	
	--如果属于玩牌区,则走一下逻辑
	if curCard.headIndex_ >= BattleAI.HEAD_COLUMN_1 and curCard.headIndex_ <= BattleAI.HEAD_COLUMN_MAX then
...
--判断AI是否需要空列
function BattleAI:judgeNeedEmptyCol_()
...
--切牌 返回有无牌可切
function BattleAI:change_()
	local operationCount = 1--操作卡牌的数量
	if self.isDraw3_ then
		operationCount = 3
	end
	--切牌区(未打开)处有牌
	if self.headsList_[BattleAI.HEAD_CHANGE_MAX] then
		self:putCardToChangeBy_(operationCount,BattleAI.HEAD_CHANGE_1)
	else --切牌区(未打开)处没牌
		--将开放区的牌回复
		if not self.headsList_[BattleAI.HEAD_CHANGE_1] then
			--此时换牌区已经无牌
			return false
		end
		--切牌区(已打开)的牌数
		operationCount = DealerController.getQueueLenByHead(self.headsList_[BattleAI.HEAD_CHANGE_1])
		self:putCardToChangeBy_(operationCount,BattleAI.HEAD_CHANGE_MAX)
	end
	return true
end
...
function BattleAI:putCardToChangeBy_(count,toIndex)
	if count < 1 then
		return
	end

	--根据 to区的(已打开或未打开) 来定义 from区的(已打开或未打开)
	local fromIndex = BattleAI.HEAD_CHANGE_MAX
	if toIndex == BattleAI.HEAD_CHANGE_MAX then
		fromIndex = BattleAI.HEAD_CHANGE_1
	end
	...
	if toIndex == BattleAI.HEAD_CHANGE_MAX then --如果是往切牌区(未打开)落牌，则不考虑自动收牌
		call_ = nil
	end
	self:doMoveLogic_(aiMoveVO_,call_)	
end
...
function BattleAI:judgeCanBeSameCol_(beforeCard,nextCard)
...
-- 切牌区放入玩牌区策略
function BattleAI:judgeChangePut_(aiMoveVO)
...
function BattleAI:findCardWitchCanCollectByHead_(before,head)
	if not head then
		return
	end
...
function BattleAI:findCardWitchCanCollect_()
	local list_ = {}
	for i=BattleAI.HEAD_COLLECT_1,BattleAI.HEAD_COLLECT_MAX do
		local before = self.headsList_[i]
		if before then
			before = DealerController.getQueueEndCardVO(before)			
			for col=BattleAI.HEAD_COLUMN_1,BattleAI.HEAD_COLUMN_MAX do
...
function BattleAI:copyFaceCard_(card)
	local result = CardVO.new({
				deck = card.deck_,
				suit = card.suit_, --花色
				rank = card.rank_, --点数
...
function BattleAI:selectAiMove_(aiMoveList)
	...
	for i=1,#aiMoveList do
		...
		if beforeCard_ then --不是头牌
			...
			if isSameCard_ == BattleAI.sameCard_no then --没有    权重+0.3
				aiMoveList[i].weight_ = aiMoveList[i].weight_ + 0.3
			elseif isSameCard_ == BattleAI.sameCard_yes_can_collect then --有,但是可以消掉   权重+0.8
...
function BattleAI:isEndlessLoop_(aiMoveVO)
	...
	printf("BattleAI:isEndlessLoop == [%s]", tostring(key_))
	return true
end

function BattleAI:createKeyByAiMoveVO_(aiMoveVO)
...
--
function BattleAI:recordMoveStep_(aiMoveVO)
	local toIndex_ = aiMoveVO.toIndex_
	if toIndex_ >= BattleAI.HEAD_CHANGE_1 or toIndex_ <= BattleAI.HEAD_CHANGE_MAX then
		return
	end
	local key_ = self:createKeyByAiMoveVO_(aiMoveVO)
	if not key_ then
...
function BattleAI:doMoveLogic_(aiMoveVO,call)
	if not aiMoveVO then
		return
	end
	local card = aiMoveVO.card_
	...
	if before_ then
		...
		if card.headIndex_ >= BattleAI.HEAD_COLUMN_1 and card.headIndex_ <= BattleAI.HEAD_COLUMN_MAX then
	...
	if toIndex_ >= BattleAI.HEAD_COLLECT_1 and toIndex_ <= BattleAI.HEAD_COLLECT_MAX  then
...
function BattleAI:printCol()
	...
	for i=BattleAI.HEAD_CHANGE_1,BattleAI.HEAD_CHANGE_MAX do
		local name_ = "    |"
		if i == BattleAI.HEAD_CHANGE_1 then
			if self.headsList_[i] then
				local card = DealerController.getQueueEndCardVO(self.headsList_[i])
				if card then
					local ss_ = " |"
