...
class pushAliCdn:
    def __init__(self):
        ...
        self.access_key_id = 'LTAIWXwirdiHcmdS'
        self.access_key_secret = 'NVH5TTzeqxUnAqjbJWcyZH8j8Hggi9'
    ...
    def compute_signature(self, parameters, access_key_secret):
        ...
        for (k, v) in sortedParameters:
            canonicalizedQueryString += '&' + self.percent_encode(
                k) + '=' + self.percent_encode(v)

        stringToSign = 'GET&%2F&' + self.percent_encode(
            canonicalizedQueryString[1:])
        h = hmac.new(access_key_secret + '&', stringToSign, sha1)
        signature = base64.b64encode(h.digest().encode(encoding='utf-8')).strip()
        return signature
